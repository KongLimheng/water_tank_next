...
export async function PUT(
  ...
  try {
    ...
    try {
      keptImages = existingGalleryRaw ? JSON.parse(existingGalleryRaw) : []
    } catch {
      // Ignore parse error
    }
    ...
    // Sync product price from first variant if variants exist
    const productPrice =
      parsedVariants.length > 0 ? parseFloat(parsedVariants[0].price) : price
    ...
    await Promise.all(imagesToDelete.map((url) => cleanImage(url, 'products')))
